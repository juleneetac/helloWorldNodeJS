    ...
    stages {
        ...
        stage('Analyze Code ') {
            ...
            container('seapdl-sonar-scanner'){
                    ...
                    script {
                        sh "sonar-scanner -Dsonar.host.url=http://seat-sonarqube.seat-ci-cd.svc.cluster.local:9000 -Dsonar.login=${SONAR_TOKEN} -Dsonar.projectName=${SONAR_PROJECT} -Dsonar.projectKey=${SONAR_PROJECT}"
                    }
                }
            }
        }
        ...
        stage('Deploy zip to Nexus') {
            steps {
                ...
                container('python') {
                    ...
                    sh "curl --header 'Content-Type: application/x-7z-compressed' --upload-file '$APP'-'$VERSION'.zip -u '$NEXUS_USERNAME':'$NEXUS_PASSWORD' -v 'http://seat-nexus3.seat-ci-cd:8081/repository/$repoName/'"
                }
            }
        }
